...
class CurlMulit {
    ...
    function remote($tmpurls, $reffer = null, $header = true,$charset=null,$encoding="",$httpheader="") {
        $urls = array();
...
//         $nurl[$k]= preg_replace('~([^:\/\.]+)~ei', "rawurlencode('\\1')", $v);
            ...
            //代理
//            $redis = Application::newLib('SRedis', true );
//            $ipcount = $redis->zcard('ippools');
//
//            $s = rand(1,$ipcount);

//            $ips = $redis->zrange('ippools',$s,$s);
//            $proxy = $ips[0];
//
////           $proxy = '121.14.138.56:81';
//            echo $proxy."\n";
//            curl_setopt ($curl[$k], CURLOPT_PROXY, $proxy);
//            curl_setopt ($curl[$k], CURLOPT_PROXYTYPE, CURLPROXY_HTTP);
            if($encoding)
            ...
            $timeout = isset($config[elements::FETCHTIMEOUT])?$config[elements::FETCHTIMEOUT]:10;
...
//            		$text[$k] = mb_convert_encoding($texttmp, "utf-8",$charset);
    ...
    //加入xpath方法
    function getRegexpInfo2($pattern, $content)
    {
        if (!strlen($pattern)){
            return null;
        }
    ...
    //加入xpath方法
    function getNodesByPage($pattern, $content)
    {
        if (!strlen($pattern)){
            return null;
        }
        ...
        return array('nodes'=>$nodes,'xpath'=>$xpath);
    }
}
